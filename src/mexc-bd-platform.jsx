  ...
  const runScout = async (rawHandle) => {
    ...
    try {
      ...
      const prompt = isWebsite
        ? "You are a crypto BD researcher. Find ALL contact information for the crypto project at website " + h + ".\n\nSEARCH STRATEGY:\n1. Search '" + h + " contact email' to find their contact page in search results\n2. Search '" + h + " bd partnerships listing' for BD-specific contacts\n3. Search '" + h + " team founder linkedin' for individual team members\n4. Check BSCScan / Etherscan if it's a token project\n5. Read the search result snippets carefully — emails are often in the snippet preview\n\nCRITICAL RULES FOR EMAILS:\n- ONLY include an email if you literally saw it written in a search result snippet or page title.\n- DO NOT guess emails based on the domain (e.g. do NOT invent 'bd@' or 'partnerships@' or 'media@' just because they sound plausible).\n- DO NOT pattern-match — if you didn't see the exact email string in your search results, the value MUST be \"Unknown\".\n- It is FAR better to return \"Unknown\" than to invent an email that might not exist.\n- The same applies to contacts[]: only include email field if you saw that specific email in search results.\n\nReturn ONLY raw JSON:\n{\"projectName\":\"Full Name\",\"symbol\":\"TICKER\",\"emoji\":\"🚀\",\"tagline\":\"one line description\",\"description\":\"2-3 sentences about the project\",\"category\":\"DeFi|Layer 1|Layer 2|AI|DePIN|RWA|Infra|Other\",\"stage\":\"Pre-Launch|Post-Launch|Listed\",\"chain\":\"chain name\",\"website\":\"" + h + "\",\"twitter\":\"@handle or Unknown\",\"telegram\":\"t.me/x or Unknown\",\"bdEmail\":\"ONLY a real email seen in search results, otherwise Unknown\",\"bdTelegram\":\"t.me/x or Unknown\",\"bestContactPath\":\"specific actionable recommendation\",\"outreachStrategy\":\"2-3 sentence exchange listing pitch\",\"bdScore\":75,\"listingInterest\":\"High|Medium|Low\",\"dataQuality\":\"High|Medium|Low\",\"contacts\":[{\"name\":\"Full Name\",\"role\":\"exact role\",\"email\":\"ONLY if seen verbatim, otherwise Unknown\",\"twitter\":\"@handle or Unknown\",\"linkedin\":\"url or Unknown\",\"telegram\":\"@handle or Unknown\",\"confidence\":\"High|Medium|Low\",\"bestPath\":\"Email|Twitter DM|Telegram|LinkedIn\",\"notes\":\"specific tip on how to reach them\"}]}"
        : "You are a crypto BD researcher for an exchange listing team. Research the project @" + h + " thoroughly.\n\nSEARCH STRATEGY:\n1. Search '" + h + " crypto official website' to find their domain\n2. Search '" + h + " contact email bd partnerships' for BD contacts\n3. Search '" + h + " team founder linkedin' for individual team members\n4. Search '" + h + " listing exchange contact' for listing-team specific info\n5. Read the search result snippets carefully — emails are often in the snippet preview\n\nCRITICAL RULES FOR EMAILS:\n- ONLY include an email if you literally saw it written in a search result snippet or page title.\n- DO NOT guess emails based on the domain (e.g. do NOT invent 'bd@' or 'partnerships@' or 'media@' just because they sound plausible).\n- DO NOT pattern-match — if you didn't see the exact email string in your search results, the value MUST be \"Unknown\".\n- It is FAR better to return \"Unknown\" than to invent an email that might not exist.\n- The same applies to contacts[]: only include email field if you saw that specific email in search results.\n\nReturn ONLY raw JSON:\n{\"projectName\":\"Full Name\",\"symbol\":\"TICKER\",\"emoji\":\"🚀\",\"tagline\":\"one line description\",\"description\":\"2-3 sentences about what they do\",\"category\":\"DeFi|Layer 1|Layer 2|AI|DePIN|RWA|Infra|Other\",\"stage\":\"Pre-Launch|Post-Launch|Listed\",\"chain\":\"chain name\",\"website\":\"domain.com\",\"twitter\":\"@handle\",\"telegram\":\"t.me/x or Unknown\",\"bdEmail\":\"ONLY a real email seen in search results, otherwise Unknown\",\"bdTelegram\":\"t.me/x or Unknown\",\"bestContactPath\":\"specific actionable recommendation\",\"outreachStrategy\":\"2-3 sentence exchange listing pitch\",\"bdScore\":75,\"listingInterest\":\"High|Medium|Low\",\"dataQuality\":\"High|Medium|Low\",\"contacts\":[{\"name\":\"Full Name\",\"role\":\"exact role\",\"email\":\"ONLY if seen verbatim, otherwise Unknown\",\"twitter\":\"@handle or Unknown\",\"linkedin\":\"url or Unknown\",\"telegram\":\"@handle or Unknown\",\"confidence\":\"High|Medium|Low\",\"bestPath\":\"Email|Twitter DM|Telegram|LinkedIn\",\"notes\":\"specific tip\"}]}";
      ...
      if (parsed.website && parsed.website !== "Unknown" && parsed.website !== "") {
        ...
        try {
          ...
          // CLEAN UP CONTACTS ARRAY — wipe any AI-invented emails that aren't in the scraped set.
          // This prevents fake bd@, partnerships@, media@ etc. from leaking through.
          // We collect ALL real emails found across the candidate pages, not just the first.
          var allRealEmails = new Set();
          // Re-collect all real emails (foundEmail was just the first/best one)
          for (var pi2 = 0; pi2 < candidatePaths.length; pi2++) {
            try {
              var pt = await fetchPage(rootUrl + candidatePaths[pi2]);
              if (pt) {
                extractEmails(pt).filter(function(e){ return !isJunkEmail(e); })
                  .forEach(function(e){ allRealEmails.add(e.toLowerCase()); });
              }
            } catch(_) {}
          }
          if (parsed.contacts && Array.isArray(parsed.contacts)) {
            var wipedCount = 0;
            parsed.contacts = parsed.contacts.map(function(c) {
              if (c && c.email && c.email !== "Unknown" && !allRealEmails.has(c.email.toLowerCase())) {
                wipedCount++;
                console.log("[Scout enrichment] Wiped unverified contact email:", c.email);
                return Object.assign({}, c, { email: "Unknown" });
              }
              return c;
            });
            if (wipedCount > 0) {
              addLog("⚠️ Wiped " + wipedCount + " unverified email" + (wipedCount > 1 ? "s" : "") + " from contacts (AI hallucination guard)");
            }
          }
        } catch (enrichErr) {
